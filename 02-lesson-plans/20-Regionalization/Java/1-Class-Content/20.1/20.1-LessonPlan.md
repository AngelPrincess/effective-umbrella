...
* General learning objectives for this unit are available in the [Objectives document](../../Objectives.md). Java-specific learning objectives include:
...
* Slack out the [learning objectives](../../Objectives.md) to students before the end of class, so they know _precisely_ what is expected of them.
...
* Open up [1-Console-IO/examples.java](Activities/1-Console-IO/Examples/console/ConsoleIO.java)
...
* Point out that we kick off the meat of our file with `public class ConsoleIO`. 
  ...
  * Explain that this creates a class, called ConsoleIO, which is _publicy visible_. In other words, it creats a class that you can use in _any_ Java code.
...
* Finally, demonstrate how to compile and run Java.
  ...
  * To run the compiled ConsoleIO.class file, run: `java -cp ../ console.ConsoleIO`
    ...
    * Explain that `-cp`—which stands for classpath—tells Java where to look for package and class that you specify afterwards.
...
* Slack out the [examples file](Activities/1-Console-IO/Examples/console/ConsoleIO.java) so students can refer to it as they work through the exercise.

* Slack out the [ConsoleIO.java](Activities/1-Console-IO/Unsolved/console/ConsoleIO.java) file to students. It contains instructions, so you don't need to slack these out separately.
...
* Open up [ConsoleIO.java](Activities/1-Console-IO/Solved/console/ConsoleIO.java). This activity is fairly straightforward, and is mainly intended to help students get some practice with Java's syntax.
...
* Open up [2-Arrays-Maps-Conditions/collections/Arrays.java](Activities/2-Arrays-Maps-Conditions/Examples/collections/Arrays.java).
...
* Open up [2-Arrays-Maps-Conditions/collections/Arrays.java](Activities/2-Arrays-Maps-Conditions/Examples/collections/ArrayLists.java)
...
* Next, we'll cover Java's HashMaps. Open up [2-Arrays-Maps-Conditions/collections/HashMaps.java](Activities/2-Arrays-Maps-Conditions/Examples/collections/HashMaps.java).
...
* Finally, open up the [conditionals example file](Activities/2-Arrays-Maps-Conditions/Examples/collections/Conditionals.java).
...
* Students are likely to be a bit overwhelmed by all the new syntax. Reassure them that this is natural, and that muscle memory comes with practice...Which the next activity will give them ample amounts of.
  ...
  * Slack out the [zip containing the collections package](Activities/2-Arrays-Maps-Conditions/Examples/collections.zip), and tell students to refer to them as they work through the activity.

### 6. Partners Do: Indefinite Student Profiles  (0:15)

* Slack out the [unsolved activity file](Activities/2-Arrays-Maps-Conditions/Unsolved/console/ConsoleIO.java). The file contains instructions, so there is no need to slack them out separately.
...
* Open up the [solution file](Activities/2-Arrays-Maps-Conditions/Solved/console/ConsoleIO.java)
...
* Slack out the [solution file](Activities/2-Arrays-Hashes-Conditions/Solved/console.zip) before moving on.
...
* Open up [the functions example file](Activities/3-Functions/Examples/functions/Functions.java).
...
* You'll have to add `static` after the access modifier for each function in the example file for it to compile.
  ...
  * For your convenienec, there's a file in the same directory, [\_Functions.java](Activities/3-Functions/Examples/functions/Functions.java), in which this keyword is already present.
...
* Slack out the [unsolved activity](Activities/3-Functions/Unsolved/functions/ConsoleIO.java). Instructions are included in the file, so there's no need to slack them out separately.
...
* Open up the [solved activity](Activities/3-Functions/Solved/console/ConsoleIO.java).
...
* Slack out the [solution zip](Activities/3-Functions/Solved/solution.zip) before moving on.
...
* Open up the [User.java example file](Activities/4-Classes-and-Objects/Examples/classes/User.java).

* Explain that the `public class` syntax that students have sseen so much of, predictably, tells Java to create a **class**.
...
* Open up the [Classes.java example file.](Activities/4-Classes-and-Objects/Examples/classes/Classes.java)
...
* Slack out the [unsolved activity](Activities/4-Classes-and-Objects/Unsolved/console/ConsoleIO.java). The file contains instructions, so there is no need to slack these out separately.
...
* Open up [your reference soltion](Activities/4-Classes-and-Objects/Solved/console/Student.java), and check your students' solution against it.

* Repeat the above procedure for the Roster class.

  * This is just a simle wrapper around ArrayList.
...
* Open up [your reference solution](Activities/4-Classes-and-Objects/Solved/console/ConsoleIO.java), and explain:
...
* Slack out the [solution zip](Activities/4-Classes-and-Objects/Solved/solution.zip) before dismissing class.
