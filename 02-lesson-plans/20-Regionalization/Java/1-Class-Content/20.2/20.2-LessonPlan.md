...
* Remind students that the last activity of their previous class required them to implement a [Student class](../20.1/Activities/), and that using classes to build our programs is a basic practice of object-oriented programming (OOP).
...
* Zip and slack out [unsolved](Activities/1-Shapes/Unsolved), and instruct students to unzip it somewhere on their computers.
...
* Open up [your solutions](Activities/1-Shapes/Solved).
...
* Explain that, to create a subclass, we use the `extends` keyword.
  ...
  * Open up your [examples](Activities/2-Subclasses/Examples/com/examples).
...
* Next, open [Dog.java](Activities/2-Subclasses/Examples/com/examples/Dog.java)
...
* Briefly, open [Cat.java](Activities/2-Subclasses/Examples/com/examples/Cat.java).
...
* Open up [Test.java](Activities/2-Subclasses/Examples/com/examples/Test.java).
...
* Zip and slack out the [unsolved](Activities/2-Subclasses/Unsolved) folder for this activity.
...
* Open up your [solution directory](Activities/2-Subclasses/Solved/com/shapes).
...
* Open up your solution [Shape.java](Activities/2-Subclasses/Solved/com/shapes/Shape.java), and briefly talk through how it hits each of these points.
...
* Zip and slack out [solved](Activities/2-Subclasses/Solved) for student reference.
...
* Point out that our abstract `Shape` class provides behavior for classes that extend it—our `Square` class gets a `perimeter` method for free, for example.

  * Explain that this is called **implementation inheritance**—subclasses _inherit_ an implementation of `perimeter`.
...
* Explain that Java has a tool that lets us _only_ define what behaviors a subclass should have, _without_ providing _any_ implementations.
  ...
  * Explain that these tools are caled _interfaces_.
...
* Open up [AnimalOld.java](Activities/3-Interfaces/Examples/com/examples/Animal.java) in 3-Interfaces.
...
* Open up [Pet.java](Activities/3-Interfaces/Examples/com/examples/Pet.java).
...
* Next, open up [Dog.java](Activities/3-Interfaces/Examples/com/examples/Dog.java).
...
* Open up [Cat.java](Activities/3-Interfaces/Examples/com/examples/Cat.java), and demonstrate that we've added the same field and method there.
...
* Open up [Test.java](Activities/3-Interfaces/Examples/com/examples/Test.java), and demonstrate that we still write our client code the same way.
...
* Explain that a "collection" is any data structure that you can put multiple objects in.
  ...
  * Point out that all of these collections have certain things in comon—like the ability to add elements, or check for emptiness—but that each collection does these things differently.
...
* After reviewing the exercise, congratulate students on having made it through a tough lesson.
  ...
  * Encourage students to think about these design problems _every time_ they have to build a new application. The brief questions and guidelines in [Things to Remember](Supplements/ThingsToRemember.md) are useful starting points at this stage.

  * Remind students that today's lesson introduced the [Four Principles of OOP](Supplements/FourPrinciplesOfOOP.md), which can be important for interviews and reading more about OOP.
  ...
  * Remind students that the real advantage to learning principles of design is the fact that they can use them in _any_ language, on _any_ project.
    ...
    * But, point out that we _can_ apply other principles—we can create and compose small, highly specialized objects to build big systems out of small pieces and we can use prototypes to share methods, for example.

    * Finally, point out that most other languages students will use later—Python, Ruby, PHP, C#, etc.—have classes very similar, if not identical, to Java's. In these languages, they can apply these principles _directly_.
