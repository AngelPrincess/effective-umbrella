...
* Open up [Throw.java](Activities/1-Exceptions/Examples/com/exceptions/Throw.java).
...
![Note that Java forces us to handle the exception we threw.](Images/1-must-handle-exceptions.png)
...
* Slack out [unsolved](Activities/1-Exceptions/Unsolved).
...
* Ask a student to explain what happened when they tried to use their `Polygon` outside of the `try` block.
  ...
  * Explain that this is because, if the code in the `try` block throws, the `Polygon` _won't_ be initialized—even if the `catch` block recovers the exception successfully—so it isn't in scope for the rest of the program.
...
* Have everyone open up `Polygon.java`, and scroll down to the custom exceptions.
  ...
  * Explain tha all we need to do is move it into its own class, and remove the `static` keyword.
...
* Finally, point out that instantiating a `Polygon` with bad data is really a programming mistake.
  ...
  * We can't really `catch` the exception and fix the proble—it's just up to the programmer not to screw up.
...
* Explain that Java provides an easy way to define these functions. Open up [Varargs.java](Activities/2-Varargs/Examples/com/varargs/Varargs.java) to demonstrate.
...
* Explain that all `Collections` have a method, called `stream`, which turns the collection into a _stream_.
  ...
  * To explain the difference, explain or act out the following scenario:
    ...
    * Fill a bag with items. It can be anything—pens, pen caps, phones, gold nuggets, anything.
    ...
    * Point out that, in this case, the student didn't use a `for` loop on the `Set`, and call `name` on each item. Instead, they _just_ called `name` on each item—someone else took care of looping.
...
* Slack out [unsolved](Activities/2-Varargs/Unsolved).
...
* Show students your [solution Streams.java](Activities/2-Varargs-Solved/com/varargs/Streams.java) for comparison.
...
* Explain that this sounds weird in English, but is straightforward to program with.
  ...
  * Explain the updated `helloDoctors` method in [Streams.java](Activities/3-Stream-Methods/Examples/com/varargs/Streams.java).
...
* Explain the `Stream.concat` example in [Streams.java](Activities/3-Stream-Methods/Examples/com/streams/Streams.java)..
...
* Scroll down to `_average`. Explain each line.
  ...
  * The second line is the call to `reduce`.
    ...
    * Point out that `reduce` receives two parameters: 0, and a lambda expression. Explain the lambda expression, first.
      ...
      * Explain that the first argument to `reduce`—0—is the value that `total` has when `reduce` first starts.
...
* Slack out [unsolved](Activities/3-Stream-Methods/Unsolved).
...
![Prompting a user for a name.](Images/10-user-prompt.png)
...
* Before dismissing class, explain that the _real_ advantage to learning these stream operations is the fact that they're useful in _many_ more places than Java.
  ...
  * To illustrate, open up [streams.js](Activities/3-Stream-Methods/Solved/streams.js).
    ...
    * Briefly show students the JavaScript version of the program they just wrote.
      ...
      * Don't belabor the point or explain the details of arrow functions—just make it clear that we can process lists with exactly the same logic in JavaScript.

    * Slack out [streams.js](Activities/3-Stream-Methods/Solved/streams.js), and encourage students to practice stream programming in both JavaScript and Java whenever they have the chance. It's a powerful technique, and just requires a bit of practice to feel natural.
