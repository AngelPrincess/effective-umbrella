...
* Use the slides provided to walk students through the core functions common to most servers:
  ...
  * The ability to send HTML or send JSONs in response to user's requesting data.
...
* Run `Activities/02-Two-Servers/server.js` and demonstrate how the message differs when you visit `http://localhost:7000` vs `http://localhost:7500`.
...
* Slack a copy of the completed app to the class: `Activities/02-Two-Servers/server.js`.
...
* Open the code for `Activities/04-Serving-HTML/server.js` and give students a minute to look it over. Ask them what they think is going to happen when you visit `localhost:80`?

![Serving HTML](Images/03-Serving-HTML.png)

* Run `Activities/04-Serving-HTML/server.js` and open `localhost:80` in your browser. It's a website!

* Point out how, in this example, we used the `fs` package to read in the `index.html` file. We then used the the node server to output this same file back to the user as a response.
...
### 11. Instructor Do: Review Students Serve HTML (0:10)
...
* Open `Activities/05-Request-Methods/server.js` in sublime.
...
* Slack students a copy of this server code: `Activities/05-Request-Methods/server.js`.
...
* In your discussion:
  ...
  * Lightly discuss the concept of a `POST` route, specifically mentioning how POST routes are effectively endpoints that client-side code can use to send code to the server.
