## 12.3 Lesson Plan - My-Massive-SQL
...
### 2. Everyone Do: Examining The Dataset (10 mins)
...
### 4. Everyone Do: Going Over the Database (10 mins)
...
* Find the csv file you want to import within the file browser and then double-click.
  ...
  * If you don't want to deal with the full 5000 rows of data and want to go with something a touch more manageable, we have also provided you with [Top1000Songs.csv](Activities/1-Top5000Schema/Top1000Songs.csv) which holds significantly less data inside of it.
...
* **Files**
  ...
  * [TopSongs.csv](Activities/1-Top5000Schema/TopSongs.csv)

  * [Top1000Songs.csv](Activities/1-Top5000Schema/Top1000Songs.csv)
...
* Open up [TopSongsCode](Activities/2-Top5000Code/topSongsCode.js) within Sublime and go over it with the class, going over the steps we used to create the database and import our table-data into it once more as well so that your students are 100% clear on how this process works.
...
* Briefly explain the notion of **indexes**.
  ...
  * Explain that instructing MySQL to use certain columns as _indexes_ helps speed up the evaluation of such queries by limiting the number of records MySQL has to examine.
    ...
    * Rather than read a table row-by-row to execute a `WHERE` clause, MySQL can use indexes to quickly seek to the relevant row in the database.
  ...
  * Explain that we can manually set arbitrary columns as indexes using the `ADD INDEX` statement.
    ...
    * Briefly demonstrate the syntax for this command on-screen: `ALTER TABLE`top5000`ADD INDEX`position`(`position`)`

  * Explain that MySQL automatically creates an index associated with a table's primary key.

  * Explain that the one determines the appropriate indexes for a table on a case-by-case basis, and that we won't dive into the guidelines just now.

    * Emphasize that this doesn't make a demonstrable difference for databases of the size we'll work with in class, but that it might for large databases that students might meet on the job.

### 9. Students Do: Two Tables Are Better Than One (60 mins)

* Once all questions have been answered, slack out the following activity...

* **Files**

  * [TopAlbums.csv](Activities/3-TwoTables/TopAlbums.csv)
...
### 10. Everyone Do: Going Over Two Tables (15 mins)

* Open up [TopSongsAndAlbumsCode](Activities/3-TwoTables/topSongsAndAlbumsCode.js) and go over the function which was added to the bottom. The query down there is far longer and far more confusing than the rest, so go over it step-by-step in great detail so that everyone in your class understands what it does.
