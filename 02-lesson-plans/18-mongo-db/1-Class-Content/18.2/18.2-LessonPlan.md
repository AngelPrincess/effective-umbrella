## 18.2 Lesson Plan - Web Scraping and Server-Side Mongo
...
* If you're using a different machine then you were in the last class, make sure you have MongoDB and RoboMongo set up. Consult the installation guide in 18.1 if you need instructions.
...
* Tell the students that you're going to demonstrate how to use Cheerio, and that they should follow along with you. Zip up activity [1-Scraping](Activities/1-Scraping) and slack it out to the class.
...
* Slack Out the skeleton of [2-Scrape-Starter](Activities/2-Scrape-Starter) to your class. Here are the instructions from the file, for reference's sake.


```
Students: Using this skeleton, the cheerio documentation, 
and what you've learned in class so far, scrape a website of 
your choice, save it in a result array, and log it to the console.
```
...
* Ask students to send their solved `server.js` files over slack, as well as the http link of the site they scraped. Choose one or two of these to go over in class.
  * Drag the solutions from your students into the directory you used for `1-Scraping`, as this will have the node packages that their `server.js` scripts need in order to function.
...
* Load up `mongod` and the `mongo` shell on your computer, and enter in five animals with the same details listed out in the last activity. If you'd like, you can also follow the queries in [3-Insert-Animals](Activities/3-Insert-Animals).
...
* Go through [4-Sorting](Activities/4-Sorting) line by line with the students. Ask them to try the commands you're explaining with their own collections. 
...
* Load the solution of [5-MongoDB-and-Sorting](Activities/5-MongoDB-and-Sorting) on your machine, and run `server.js` with Node. Visit the different routes in your web browser to show students the results:

  * `/` will display a simple hello world message.
  * `/all` will display a json with every animal in your zoo collection.
  * `/name` will display a json with every animal, sorted by name.
  * `/weight` will display a json with every animal, sorted by weight.

* Slack Out the skeleton of [5-MongoDB-and-Sorting](Activities/5-MongoDB-and-Sorting) to your class. Here are the instructions from the file, for reference's sake.
...
* Open the solution to [6-MongoJS-and-the-Front-End](Activities/6-MongoJS-and-the-Front-End) on your machine and run the `server.js` file. Note how it pulls in the information you typed in earlier for your zoo collection, and places it in an html table. 
...
* With the finished activity demonstrated, slack out the skeleton of [6-MongoJS-and-the-Front-End](Activities/6-MongoJS-and-the-Front-End), and ask them to open up the `public/app.js`. This is where they'll find these instructions:
...
* On your machine, open up the solution of [7-Scraping-into-a-db](Activities/7-Scraping-into-a-db) and demonstrate the working file, which will not only scrape a website, but also place the website data in a MongoDB database.

  * To that end, open up RoboMongo and find the data in the collection that `server.js` just made (which should be under the "scraper" database as "scrapedData").

* Slack out the skeleton of [7-Scraping-into-a-db](Activities/7-Scraping-into-a-db). Here are the instructions students will see:
...
* Slack out the skeleton of [8-Note-Taker](Activities/8-Note-Taker), and tell students that if they finish activity 5, they should attempt to complete this Note-Taker app. 

* Tell students that even if they don't get to finish the app, or even if they don't finish assignment 5, they should still try to finish this before Saturday. "It's not mandatory, but it will be good practice before your homework assignment."
