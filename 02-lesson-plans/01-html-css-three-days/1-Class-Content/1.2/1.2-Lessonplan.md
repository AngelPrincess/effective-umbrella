...
* Have your TAs reference [1.2-TimeTracker](1.2-TimeTracker.xlsx) to help keep track of time during class.
...
* Open the file `basic-html-bio.html` in your browser.
...
* Explain the `div` and `section` tags that appear in the solution.
  ...
  * Encourage students to use a `div` when they're not sure which tag to choose.
...
* Take about 15 minutes to quickly demonstrate a few examples of CSS styling. As a starting point you can use the files inside `2-BasicCSS` to showcase the pieces necessary to link an HTML page to CSS.
...
* Open the file `html-bio-with-css.html` (`3-HTML_CSS_Layout`) in your browser. Warn students that this might be a tough activity for some of them.

    ![3-CSS_Layout_1](Images/3-CSS_Layout_1.png)

* Slack out [html-bio-with-css.html](Activities/3-HTML_CSS_Layout/html-bio-with-css.html) as a starter file.
